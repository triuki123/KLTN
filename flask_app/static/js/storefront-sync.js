...
(() => {
  ...
  const cover = (url, title) => url ? `<img src="${escapeHtml(url)}" alt="Bìa sách ${escapeHtml(title)}">` : `<div class="cover-placeholder">${escapeHtml(title?.charAt(0) || 'S')}</div>`;
  ...
  function renderBooksPage() {
    root.innerHTML = `<section class="shell"><span class="eyebrow">Thư viện mở Open Library</span><div class="section-head"><h1 class="page-title">Khám phá kho sách</h1></div><p>Tìm theo tên sách, tác giả hoặc ISBN. Nhập nội dung bạn muốn tìm — không còn từ khóa mặc định.</p><form class="search" id="book-search"><input id="book-query" placeholder="Ví dụ: Harry Potter, Dế Mèn, ISBN…" autocomplete="off"><button class="btn-brand">Tìm sách</button></form><p id="book-result" class="mt-4">Hãy nhập từ khóa để bắt đầu tìm sách.</p><div id="book-results" class="book-grid"></div></section>`;
    document.querySelector('#book-search').addEventListener('submit', async event => {
      ...
      try {
        ...
        document.querySelector('#book-result').textContent = `Tìm thấy ${Number(data.total || 0).toLocaleString('vi-VN')} kết quả cho “${query}”.`;
